...
class Player {
    ...
    public void respawn(boolean ded, boolean godMode) {
        if (godMode){
            respawnTimer=0;
            return;
        }
...
//        }
    ...
    public Vector updatePhysics(ArrayList<Tile> collide, int timeStep, Level currentLevel, double tileSize, GraphicsContext gc, boolean godMode, byte[] arrows, boolean playTest, double frameTime) {
        double multiplier = (frameTime/0.01649779880834474)*1.2;
        this.pos.mult(tileSize);
        if (this.pos.y>(currentLevel.t[0].length*tileSize) && respawnTimer<=0) respawn(true, godMode);
        double scrollX;
        double scrollY;
        jumpTimer--;
 ...
 Light the player
        try {
            currentLevel.t[(int) Math.floor(this.pos.x / tileSize)][(int) Math.floor(this.pos.y / tileSize)].lightIntensity = Color.hsb(0, 0, 1);
        }catch(ArrayIndexOutOfBoundsException ignored){}
*/
...
//            win.play();
        ...
        if (jumpTimer==0) {
            ...
            if (downTouch.isSolid(onoffstate)) {
                if (arrows[0]==0) {
                ...
                this.vel.add(new Vector(0, (tileSize/64f/timeStep)));
            }
        } else {
            jumpTimer--;
        }
        ...
        if (rightTouch.isCollidable(onoffstate)) {
            if (arrows[3]==0) {
        ...
        if (leftTouch.isCollidable(onoffstate)) {
            if (arrows[2]==0) {
                if (jumpTimer==0 || this.vel.x<=1e-5)
                    this.vel.set(new Vector(0, this.vel.y));
            } else {
        ...
        if (upTouch.isCollidable(onoffstate)) {
...
//                toggle.play();
        ...
        this.pos.x += ((this.vel.x/1.2)/timeStep)*multiplier;
...
//        if (upTouch.id == 8) {
//            respawnPos = new Vector(upTouch.exact.x, upTouch.exact.y);
//            checkpoint=true;
//        } else if (downTouch.id == 8) {
//            respawnPos = new Vector(downTouch.exact.x, downTouch.exact.y);
//            checkpoint=true;
//        } else if (leftTouch.id == 8) {
//            respawnPos = new Vector(leftTouch.exact.x, leftTouch.exact.y);
//            checkpoint=true;
//        } else if (rightTouch.id == 8) {
//            respawnPos = new Vector(rightTouch.exact.x, rightTouch.exact.y);
//            checkpoint=true;
//        }

//        if (debug) {
//            gc.setFill(Color.grayRgb(255));
//            gc.fillRect(upHit.x, upHit.y, upHit.w, upHit.h);
//            gc.setFill(Color.rgb(255, 0, 0));
//            gc.fillRect(downHit.x, downHit.y, downHit.w, downHit.h);
//            gc.setFill(Color.rgb(0, 255, 0));
//            gc.fillRect(leftHit.x, leftHit.y, leftHit.w, leftHit.h);
//            gc.setFill(Color.rgb(0, 0, 255));
//            gc.fillRect(rightHit.x, rightHit.y, rightHit.w, rightHit.h);
//        }

//        for (Enemy e : enemies) {
//            gc.fillRect enemyHit = new Rect(e.pos.x, e.pos.y, tileSize/2, tileSize/2);
...
//                continue;
//            }
//            if (upHit.isTouching(enemyHit) || rightHit.isTouching(enemyHit) || leftHit.isTouching(enemyHit) && respawnTimer==0) {
//                respawn(true);
//                respawnTimer=100;
//                println("died to enemy");
//            }
//        }
