                            ...
                            if(size==2){
                                ...
                                for (int i=0; i<inventoryTiles.length; i++) {
                                    ...
                                    for (int j=0; j<inventoryTiles[i].length; j++) {
                                        ...
                                        Rect hitBox = new Rect(((mainCanvas.getWidth()/1.9) - (width[0])/2)+(j*mainCanvas.getWidth()/18), ((mainCanvas.getHeight()/1.9) - (height[0])/2)+(i*mainCanvas.getHeight()/7) + mainCanvas.getHeight()/30f, mainCanvas.getHeight()/20f, mainCanvas.getHeight()/20f);
                                        tile.setHitBox(hitBox);
                                        hitBox = new Rect((((mainCanvas.getWidth()/1.9) - (width[0])/2)+(j*mainCanvas.getWidth()/18)) - ((mainCanvas.getHeight()/15f) - (mainCanvas.getHeight()/20f))/2, ((mainCanvas.getHeight()/1.9) - (height[0])/2)+(i*mainCanvas.getHeight()/7) - ((mainCanvas.getHeight()/15f) - (mainCanvas.getHeight()/20f))/2 + mainCanvas.getHeight()/30f, mainCanvas.getHeight()/15f, mainCanvas.getHeight()/15f);
                                        if (inventoryTiles[i][j]==selectedTile) {
                                            ...
                                            describe.render(gc, mainCanvas.getHeight()/10f);
                                            gc.setTextAlign(TextAlignment.CENTER);
                                            gc.setFont(Font.font("Cambay", FontWeight.BOLD, FontPosture.ITALIC, (int)(height[0])/25f));
                                            gc.setFill(Color.BLACK);
                                        ...
                                        tile.render(gc, mainCanvas.getHeight()/20f);
                                    }
                                }
                            }
                            gc.setFill(Color.WHITESMOKE);
...
//                        }
                ...
                if(editing || inventory) {
                    l.render(scrollX, scrollY, tileSize, gc, mainCanvas.getWidth(), mainCanvas.getHeight());
                        ...
                        case(2):
                            ...
                            for(int i=0;i<l.t.length;i++){
                                for(int j=0;j<l.t[i].length;j++){
                                    if(l.t[i][j].id>=17 && l.t[i][j].id<=22){
                            ...
                            try {
                                if (tileOneForWire != null) {
                                    if (tileOneForWire != l.t[MX][MY]) {
                                        if((l.t[MX][MY].id >= 17 && l.t[MX][MY].id <= 22)) {
                                            Vector newInput = new Vector(Math.round((tileOneForWire.hitBox.x-scrollX) / tileSize),Math.round((tileOneForWire.hitBox.y-scrollY) / tileSize));
                    ...
                    if(mousePressed)
                        if(mouseButton==0) {
                            ...
                            if(tool==2){
                                if(tileOneForWire==null && (l.t[MX][MY].id>=17&&l.t[MX][MY].id<=20)){
    ...
    private String[] saveLevel(Canvas mainCanvas){
        ...
        for(int j=0;j<l.t[0].length;j++) {
            for (int i = 0; i < l.t.length; i++) {
                ...
                if(l.t[i][j].id>=17 && l.t[i][j].id<=22 && l.t[i][j].inputs.length>0) {
